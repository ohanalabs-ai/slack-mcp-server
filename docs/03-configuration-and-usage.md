...
| `MCP_OAUTH_BROKER_URL`            | No        | `nil`                     | Base URL of an [ohanalabs-ai/mcp-oauth-broker](https://github.com/ohanalabs-ai/mcp-oauth-broker) instance (e.g. `http://localhost:9442`). When set, this server fetches its Slack token live from the broker's `GET /internal/credentials/latest` at startup instead of requiring a static `SLACK_MCP_XOXP_TOKEN`. Requires `MCP_OAUTH_BROKER_INTERNAL_API_KEY`. A fetch failure falls back to any statically configured token env vars rather than failing startup. This is a startup-time fetch only — a token refreshed on the broker later requires restarting this server to pick up. |
| `MCP_OAUTH_BROKER_INTERNAL_API_KEY` | No*     | `nil`                     | Bearer secret for the broker's internal credential API. Required together with `MCP_OAUTH_BROKER_URL`. |
| `MCP_OAUTH_BROKER_PROVIDER`       | No        | `slack`                   | Which provider's credential to fetch from the broker. Only relevant alongside `MCP_OAUTH_BROKER_URL`. |
| `MCP_OAUTH_BROKER_ISSUER`         | No        | `nil`                     | Base URL of the same broker used as an OAuth 2.1 issuer (e.g. `http://localhost:9442`). When set, incoming requests may authenticate with a valid broker-issued JWT (verified via `{issuer}/.well-known/jwks.json`) as an alternative to `SLACK_MCP_API_KEY` — both are independently checked; either one succeeding authenticates the request. |
