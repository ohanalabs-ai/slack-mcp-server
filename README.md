...
Model Context Protocol (MCP) server for Slack Workspaces. The most powerful MCP Slack server — supports Stdio and streamable HTTP transports, proxy settings, DMs, Group DMs, Smart History fetch (by date or count), may work via OAuth or in complete stealth mode with no permissions and scopes in Workspace 😏.
...
- **Stdio/HTTP (streamable) Transports & Proxy Support**: Use the server with any MCP client that supports Stdio or streamable HTTP transports, and configure it to route outgoing requests through a proxy if needed.
